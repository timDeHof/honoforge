...
export function extractRouteMetadata(app: OpenAPIHono): RouteMetadata[] {
  ...
  return routes.map((route) => {
    ...
    if (route.summary) metadata.summary = route.summary
    if (route.description) metadata.description = route.description
    if (route.tags) metadata.tags = route.tags
    ...
    if (route.request) {
      ...
      if (req.query) requestSchema.query = req.query
      if (req.params) requestSchema.params = req.params
      if (req.headers) requestSchema.headers = req.headers
      if (req.body) requestSchema.body = req.body
